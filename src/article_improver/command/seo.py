...
from article_improver.chat_gpt import ChatGpt, MODEL_GPT_3
...
async def handle(chat_gpt: ChatGpt, filename: str):
    content = compress(app.read_pdf(filename))
    completion = await chat_gpt.get_completion(PROMPT, content, MODEL_GPT_3)
